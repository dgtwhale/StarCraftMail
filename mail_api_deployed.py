    ...
    try:
        ...
        for data, charset in decode_header(value):
            if isinstance(data, bytes):
                out += data.decode(charset or 'utf-8', errors='ignore')
            else:
                out += data
        return out
    except Exception:
    ...
    try:
        ...
        for msg_id in message_ids:
            ...
            if status == 'OK':
                ...
                emails.append({
                    'id': msg_id.decode(),
                    'from': decode_mime(msg.get('From', 'Unknown')),
...
def register():
    ...
    try:
        ...
        with open(DOVECOT_USERS, 'a') as f:
            f.write(f"{email}:{password}:5000:5000::{vhost_dir}::\n")
